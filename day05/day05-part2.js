...
function getPointIndexesToCover(parsedPositionTuple) {
  ...
  const is45DegreesDiagonal = Math.abs(x1 - x2) === Math.abs(y1 - y2);
  ...
  } else if (is45DegreesDiagonal) {
    const xPositionsToCover = getNumbersBetweenTwoNumbers(x1, x2);
    const yPositionsToCover = getNumbersBetweenTwoNumbers(y1, y2);

    // getNumbersBetweenTwoNumbers returns lower to higher
    // reverse if the first number is higher so we don't rework the function
    if (x1 > x2) {
      xPositionsToCover.reverse();
    }

    if (y1 > y2) {
      yPositionsToCover.reverse();
    }

    return xPositionsToCover.map((e, idx) => [e, yPositionsToCover[idx]]);
  } else {
    return null;
  }
}
